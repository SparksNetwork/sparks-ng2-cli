...
var Compiler = (function () {
    function Compiler(filePath, settings) {
    ...
    Compiler.prototype.addSchema = function(schema) {
        var id = schema.title || schema.id;
        var type = this.toTsType(schema, schema, '', true);
        this.declareType(type, id, id);
    };
    ...
    Compiler.prototype.resolveRefFromSchemas = function (refPath, propName) {
        var schemas = this.settings.schemas;
        var schema = schemas[refPath];

        if (schema) {
          var targetType = this.toTsType(schema, schema, propName, true);
    ...
    Compiler.prototype.resolveRefFromLocalFS = function (refPath, propName) {
        ...
        var targetType = this.toTsType(contents, contents, propName, true);
        var id = targetType.id
            ? targetType.toType(this.settings)
    ...
    Compiler.prototype.resolveRef = function (schema, rule, refPath, propName) {
        if (refPath === '#' || refPath === '#/') {
            return TsType.Interface.reference(refPath)
        }
        if (refPath[0] !== '#') {
            return this.resolveRefFromSchemas(refPath, propName) || this.resolveRefFromLocalFS(refPath, propName);
        }
        ...
        var innerSchema = lodash_1.get(schema, parts.join('.'));
        var type = this.toTsType(innerSchema, innerSchema);
    ...
    Compiler.prototype.generateTsType = function (schema, rule, propName, isReference) {
        var _this = this;
        ...
        switch (this.getRuleType(rule)) {
            ...
            case RuleType.TypedArray: return new TsType.Array(this.toTsType(schema, rule.items));
            ...
            case RuleType.AllOf:
                return new TsType.Intersection(rule.allOf.map(function (_) { return _this.toTsType(schema, _); }));
            case RuleType.AnyOf:
                return new TsType.Union(rule.anyOf.map(function (_) { return _this.toTsType(schema, _); }));
            case RuleType.Reference:
                return this.resolveRef(schema, rule, rule.$ref, propName);
            case RuleType.Union:
                return new TsType.Union(rule.type.map(function (_) { return _this.toTsType(schema, { type: _ }); }));
        }
    ...
    Compiler.prototype.toTsType = function (schema, rule, propName, isReference) {
        if (isReference === void 0) { isReference = false; }
        var type = this.generateTsType(schema, rule, propName, isReference);
        if (!type.id) {
        ...
        type.declare = this.settings.declareTypes;
        return type;
    };
    Compiler.prototype.toTsDeclaration = function (schema) {
        ...
        var props = lodash_1.map(copy.properties, function (v, k) {
            return {
                ...
                type: _this.toTsType(schema, v, k)
            };
        });
        ...
        if (this.supportsAdditionalProperties(copy)) {
            ...
            var type = short ? new TsType.Any : this.toTsType(copy, copy.additionalProperties);
            props.push({
                name: '[k: string]',
                required: true,
                type: type
